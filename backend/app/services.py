...
def generate_history_data(range_days: int = 30, metric: str = "revenue") -> HistoryResponse:
    ...
    df["weekend_factor"] = df["day_of_week"].apply(lambda x: cfg.weekend_dip if x >= 5 else 1.0)
    df["value"] = (df["base"] + df["noise"]) * df["weekend_factor"]
    df["value"] = df["value"].clip(lower=0).round(2)
    
    data_points: List[DataPoint] = [
        DataPoint(date=row["date"].strftime("%Y-%m-%d"), value=row["value"])
        for _, row in df.iterrows()
    ]
    
    return HistoryResponse(metric=metric, range_days=range_days, data=data_points)
    ...
    for customer in selected_customers:
        ...
        transactions.append(
            Transaction(
                ...
                status=random.choices(["completado", "pendiente"], weights=[85, 15], k=1)[0],
            )
        )
    
    transactions.sort(key=lambda x: x.date, reverse=True)
    
    return TransactionsResponse(transactions=transactions, total=len(transactions))
